...
class LaunchAppInStoreService {
  final List<Map<String, dynamic>> _banksWithPackageName = [
    {
      "bankName": "Сбербанк",
    ...
    {"bankName": "Кредит Европа Банк Россия", "logoURL": "https://qr.nspk.ru/proxyapp/logo/bank100000000027.png", "schema": "bank100000000027"},
    {
  ...
  String getPackageName(BankInfo bank) {
    try {
      return _banksWithPackageName.firstWhere((element) => element['schema'] == bank.code)['package_name'];
    } catch (_) {
      return '';
    }
  }

  Future<void> launchAvailableStore({
    ...
    String? packageName;
    try {
      packageName = _banksWithPackageName.firstWhere((element) => element['schema'] == bank.code)['package_name'];
    } catch (_) {}

    if (packageName == null) return;

    final playStoreUrl = 'https://play.google.com/store/apps/details?id=$packageName';
    final ruStoreUrl = 'https://apps.rustore.ru/app/$packageName';
    final huaweiDeepLink = 'appmarket://details?id=$packageName';
    final huaweiWebUrl = 'https://appgallery.huawei.com/app/$packageName';
    ...
    try {
      final res = await http.head(Uri.parse(playStoreUrl));
      existsIn['play'] = res.statusCode == 200;
    } catch (_) {}

    try {
      final res = await http.head(Uri.parse(ruStoreUrl));
      existsIn['rustore'] = res.statusCode == 200;
    } catch (_) {}

    existsIn['huaweiWeb'] = await canLaunchUrl(Uri.parse(huaweiWebUrl));
    existsIn['huaweiDeep'] = await canLaunchUrl(Uri.parse(huaweiDeepLink));

    // Launch in priority order
    if (existsIn['play'] == true) {
      await launchUrl(Uri.parse(playStoreUrl), mode: LaunchMode.externalApplication);
      return;
    }

    if (existsIn['rustore'] == true) {
      await launchUrl(Uri.parse(ruStoreUrl), mode: LaunchMode.externalApplication);
      return;
    }

    if (existsIn['huaweiDeep'] == true) {
      await launchUrl(Uri.parse(huaweiDeepLink), mode: LaunchMode.externalApplication);
      return;
    }

    if (existsIn['huaweiWeb'] == true) {
      await launchUrl(Uri.parse(huaweiWebUrl), mode: LaunchMode.externalApplication);
      return;
    }

    debugPrint("❌ App not found in Play Store, RuStore, or AppGallery.");
  }
}
